...
캐릭터 창의 `Check` 버튼은 백그라운드에서 검증을 실행한 뒤 상태를 다시 읽는다. 실행 중에는 얼굴과 라벨이 `검증 중` 상태로 잠깐 바뀌고, 중복 클릭과 동시 갱신을 막기 위해 조작 버튼이 잠시 비활성화된다. 이 실행 중 반응은 너무 빨리 사라지지 않도록 최소 0.8초 동안 표시한다.
